...
| P10 | Shared device | Evidence-supported conclusion | Policy or logs affect another household member | Non-user privacy violation | Per-user setup where available; no household logging by default |
| P11 | Strict mode | Evidence-supported conclusion | Emergency or essential site is blocked | Safety/access harm | Always-available safety exception and allowlist; incident review |
| P12 | Strict mode | Evidence-supported conclusion | Partner/employer secretly configures or controls exit | Coercive control | Adult self-authorization, anti-coercion checks, no secret admin |
| P13 | Cloud | Evidence-supported conclusion | Sync outage or account takeover | Policy inconsistency or exposure | Local operation, secure recovery, minimal server data, transparent conflict state |
| P14 | Classification | Evidence-supported conclusion | Domain/page classifier false positive or false negative | Lockout or false confidence | Deterministic fixture, appeal, uncertainty, no clinical/enforcement guarantee |

## Failure Communication Contract

- **Design requirement:** State the adapter, declared action, result, and limitation in plain language.
- **Design requirement:** Never hide a permission loss or present a local failure as user misconduct.
- **Design requirement:** A failure event must not automatically disclose browsing intent to an ally.
...
- **Repository decision:** Reject any proposed mechanism that depends on covert installation, raw screenshot upload, breaking encryption, or unreviewed third-party control.
- **Design requirement:** Escalate entitlement, app-store policy, managed-device, work-device, and legal uncertainty rather than converting it into a capability claim.
