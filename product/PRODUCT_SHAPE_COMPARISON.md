...
| --- | --- | --- |
| User problem | Evidence-supported conclusion | Adults may need a calm, voluntary way to turn intention into a plan and one next action without surveillance or a clinical label. |
...
| Supporting evidence | Evidence-supported conclusion | Threads 01, 02, 05, 06, and 07 support a cautious testable shape; no Better Life efficacy evidence exists. |
| Required data | Design requirement | Accountless core needs only local user-authored plan/settings and optional local outcome responses. |
| Consent | Design requirement | Manual use has no external share; every future adapter/service is a separate opt-in capability. |
| Safety/misuse | Evidence-supported conclusion | Clinical overclaim, shame, coercion, lockout, false protection, sensitive data, and inaccessible exit are critical. |
...
| Exit strategy | Design requirement | Keep/remove each layer independently; stop before enforcement if manual core lacks value. |
...
| Chromium extension + local dashboard | Platform limitation | Hypothesis: visible user-configured browser friction may add value near selected domains. | Extension permissions/rules; incognito and other browser/device bypass. | False confidence, overblocking, permission concern. | Separate technical/product spike after core value and privacy gates. |
| Native Android app/VPN | Platform limitation | Hypothesis: device-level network control may add value and moment-of-need access. | VPN/service permissions, app-store policy, battery/network impact. | High trust and platform burden before need is known. | Second enforcement spike, not first MVP baseline. |
| Native iOS app | Platform limitation | Hypothesis: native UX and official-control possibilities may add value. | Entitlement/App Review/API limits. | Schedule/approval dependency and incomplete coverage. | Parallel feasibility track, not first MVP baseline. |
...
| ---: | --- | --- | --- |
| 0 | Repository decision | Interview scripts, paper/clickable plan and Spiral; no production code. | Phase 0 safety and problem evidence. |
...
- **Design requirement:** Do not describe the core as protection across devices, relapse prevention, therapy, AI coaching, or anonymous support.
